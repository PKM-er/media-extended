...
import { WithMediaState } from "../typings";
...
const UserSeekEndReducerFor =
  (source: UserSeekSource) => (state: WithMediaState) => {
...
const PreciseSeekReducerFor =
  (source: UserSeekSource) =>
  (state: WithMediaState, action: PayloadAction<number>) => {
    const priority = compareSeekPriority(source, state);
    if (priority < 0) return;
    ...
    if (priority === 0) {
      ...
      state.userSeek = {
        ...
        currentTime: time,
        source,
        pausedBeforeSeek: state.status.paused,
      };
...
const OffsetSeekReducerFor =
  (
    source: UserSeekSource,
    onUpdate: (state: WithMediaState, offset: number) => any,
  ) =>
  (state: WithMediaState, action: PayloadAction<number>) => {
    const priority = compareSeekPriority(source, state);
    if (priority < 0) return;
