...
import type { Track } from "mx-store";
 ...
 */
const getSubtitleTracks = async (subtitles: TFile[], defaultLang: string) => {
  ...
  return url.reduce<Track[]>((arr, [file, src], _i, { length: size }) => {
    ...
    if (size > 1) {
      ...
      props = {
        ...
        default: defaultLang === languageCode,
      };
    } else {
      props = {
...
const getTracks = async (video: TFile, defaultLang: string) => {
  const subtitles = getSubtitles(video);
  if (subtitles.length === 0) return [];
  return getSubtitleTracks(subtitles, defaultLang);
};
