...
export function toTempFrag(durationInSecond: number | string): string | null {
  const _duration = Number(durationInSecond);
  if (Number.isNaN(_duration)) return null;

  const duration = dayjs.duration(_duration, "seconds");
