...
Welcome to your first adventure in efficient video note-taking with Media Extended. We've made everything straightforward, so even if you're just starting, you'll find it incredibly easy to dive in and begin capturing media notes right away. Let's leave the complexity behind and enjoy a smooth, intuitive experience together.
...
- **Screenshot Button**: This button allows you to capture a screenshot of the current video frame and save it as an image file within your vault, alongside the timestamp and video link. This feature is perfect for capturing visual content that you want to reference in your notes.
...
Now you've opened your first media note, it's time to start taking notes. The Media Extended plugin provides a seamless note-taking experience, allowing you to capture your thoughts and observations in real-time as you watch the video.
...
When working with multiple notes with different media, you could find it annoying to switch between players. Media Extended solves this problem by integrating obsidian's native "pin tab" feature. Just right-click on the player tab and select "Pin". Now, this player will be shared across all notes, and you can easily switch between notes without being spammed with multiple players.
...
<Tabs items={['Local file', 'Websites']}>
  ...
  2. For other files on your computer, use [media quick switcher](#quick-switcher) by either entering path or using the file picker by invoking "Open local file".
  </Tabs.Tab>
  <Tabs.Tab>
  ...
  <Callout type="warning">
    If website requires login to access content or request login during playback, you can open a browser page to login from the plugin's settings, or Login command. 
